...
class PlayState extends MusicBeatState
{
	...
	var shootBeats2:Array<Int> = [40, 56, 72, 88, 112, 128, 168, 171, 173, 176, 179, 181, 208, 224, 232, 240, 248, 256, 264];
	...
	override public function create()
	{
			...
			case 'library-t':
				{
						defaultCamZoom = 0.9;
						curStage = 'stage';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('librarytutorial', 'librarian'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
			...
			case 'library':
				{
						defaultCamZoom = 0.7;
						curStage = 'library';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('libraryback','librarian'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
	
						var shadow:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('shadow','librarian'));
						shadow.setGraphicSize(Std.int(shadow.width * 0.9));
						shadow.updateHitbox();
						shadow.antialiasing = true;
						shadow.scrollFactor.set(1.3, 1.3);
						shadow.active = false;
                        add(shadow);

						whitty = new FlxSprite(-530, -450);
					    whitty.frames = Paths.getSparrowAtlas('whitty','librarian');
					    whitty.animation.addByPrefix('dance', 'GROVE', 24, false);
					    whitty.antialiasing = true;
					    whitty.scrollFactor.set(0.9, 0.9);
					    whitty.setGraphicSize(Std.int(whitty.width * 1));
					    whitty.updateHitbox();
					    if(FlxG.save.data.distractions){
					    	add(whitty);
					    }	
		...
		{
			case 'library':
				bottomLibrary = new FlxSprite(-600, -185);
				bottomLibrary.frames = Paths.getSparrowAtlas('CrowdTOP1','librarian');
				bottomLibrary.animation.addByPrefix('dance', 'GROVETOP', 24, false);
				bottomLibrary.antialiasing = true;
				bottomLibrary.scrollFactor.set(1.1, 1.1);
				bottomLibrary.setGraphicSize(Std.int(bottomLibrary.width * 1));
				bottomLibrary.updateHitbox();
				if(FlxG.save.data.distractions){
					add(bottomLibrary);
				}
		}
		...
		healthBar.createFilledBar(0xFF796E78, 0xFF31B0D1);
		// healthBar
		add(healthBar);
		...
		for (x in 0...shootBeats.length)
			{
				...
				warnNote.x += FlxG.width / 3; // general offset
			}
		}

		if (curSong == 'Blammed' && dad.curCharacter == "pico"){
		for (x in 0...shootBeats2.length)
			{
				//also borrowed from fnf hd 2
				var warnNote2Time = shootBeats2[x];
				var warnNote2:Note = new Note(warnNote2Time * beatStepTime, 4, null, false, true);
			    warnNote2.scrollFactor.set();
				unspawnNotes.push(warnNote2);
				warnNote2.mustPress = false;
				warnNote2.x += FlxG.width / 3; // general offset
			}
		}
		...
		for (i in 0...5)
		{
			...
			switch (SONG.noteStyle)
			{
				case 'pixel':
					...
					babyArrow.animation.add('white', [9]);
				...
				case 'library':
					babyArrow.frames = Paths.getSparrowAtlas('noteassets/NOTE_assets','librarian');
					babyArrow.animation.addByPrefix('green', 'arrowUP');
					babyArrow.animation.addByPrefix('blue', 'arrowDOWN');
					babyArrow.animation.addByPrefix('purple', 'arrowLEFT');
					babyArrow.animation.addByPrefix('red', 'arrowRIGHT');
			...
			if (!isStoryMode)
			{
				babyArrow.y -= 10;
				babyArrow.alpha = 0;
				FlxTween.tween(babyArrow, {y: babyArrow.y + 10, alpha: 1}, 1, {ease: FlxEase.elasticInOut, startDelay: 0.5 + (0.2 * i)});
			}
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.modifiedByLua)
						{
							...
							{
								if (daNote.mustPress)
									daNote.y = (playerStrums.members[Math.floor(Math.abs(daNote.noteData))].y + 0.45 * (Conductor.songPosition - daNote.strumTime) * (0.45 * (daNote.deathNote?1.25:1) * (0.45 * (daNote.deathNote2?1.25:1) * FlxMath.roundDecimal(FlxG.save.data.scrollSpeed == 1 ? SONG.speed : FlxG.save.data.scrollSpeed, 2))));
								else
									daNote.y = (strumLineNotes.members[Math.floor(Math.abs(daNote.noteData))].y + 0.45 * (Conductor.songPosition - daNote.strumTime) * (0.45 * (daNote.deathNote?1.25:1) * (0.45 * (daNote.deathNote2?1.25:1) * FlxMath.roundDecimal(FlxG.save.data.scrollSpeed == 1 ? SONG.speed : FlxG.save.data.scrollSpeed, 2))));
					...
					if ((daNote.mustPress && daNote.tooLate && !FlxG.save.data.downscroll || daNote.mustPress && daNote.tooLate && FlxG.save.data.downscroll) && daNote.mustPress)
					{
						...
						if (daNote.deathNote && (SONG.song.toLowerCase() == 'blammed' ||SONG.song.toLowerCase() == 'milf'))
						{
							health -= 1;
						} 
						else
	...
	public function NearlyEquals(value1:Float, value2:Float, unimportantDifference:Float = 10):Bool
		...
		var middleHold:Bool = false;
		var rightHold:Bool = false;
		var leftHold:Bool = false;	

		private function keyShit():Void // I've invested in emma stocks
			{
				// control arrays, order L D M R U
				var holdArray:Array<Bool> = [controls.LEFT, controls.DOWN, controls.MIDDLE ,controls.UP, controls.RIGHT];
				...
				if (controls.MIDDLE_P){luaModchart.executeState('keyPressed',["middle"]);};
	...
	{
		if (!boyfriend.stunned && !daNote.deathNote2)
		{
			health -= 0.04;
	...
	override function beatHit()
	{
		...
		if (curSong == 'Blammed' && dad.curCharacter == "pico" && !inCutscene && !gameEnd)
		{
			if(shootBeats.contains(curBeat)){
				FlxG.sound.play(Paths.sound("CLAP", "shared"), 1);
		...
		if (curSong == 'Blammed' && dad.curCharacter == "pico" && !inCutscene && !gameEnd)
			{
				if(shootBeats2.contains(curBeat)){
					FlxG.sound.play(Paths.sound("CLAP", "shared"), 1);
					...
					if (FlxG.keys.justPressed.J || FlxG.keys.justPressed.RIGHT)
						{
							health -= 2;
						}
				}
		...
		switch (curStage)
		{
			...
			case 'library':
				if(FlxG.save.data.distractions){
					whitty.animation.play('dance', true);
					bottomLibrary.animation.play('dance', true);
				}	
