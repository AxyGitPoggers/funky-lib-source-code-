...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		var middle = FlxG.keys.justPressed.THREE;
		var up = FlxG.keys.justPressed.FOUR;
		var right = FlxG.keys.justPressed.FIVE;
		var leftO = FlxG.keys.justPressed.SIX;
		var downO = FlxG.keys.justPressed.SEVEN;
		var middleO = FlxG.keys.justPressed.EIGHT;
		var upO = FlxG.keys.justPressed.NINE;
		var rightO = FlxG.keys.justPressed.P;

		var pressArray = [left, down, middle, up, right, leftO, downO, middleO, upO, rightO];
		var delete = false;
	...
	{
		remove(gridBG);
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 10, GRID_SIZE * _song.notes[curSection].lengthInSteps);
        add(gridBG);
		...
		for (i in sectionInfo)
		{
			...
			var note:Note = new Note(daStrumTime, daNoteInfo % 5);
